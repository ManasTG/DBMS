...
MariaDB [manas_db]> CREATE TABLE emp_cp3 AS SELECT * FROM emp;
Query OK, 14 rows affected
Time: 0.038s
MariaDB [manas_db]> SELECT* FROM emp_cp3;
+-------+--------+-----------+--------+------------+------+--------+--------+
...
MariaDB [manas_db]> DELETE FROM emp_cp3 
...
MariaDB [manas_db]> SELECT e.ename, e.job, d.dname, d.location
-> FROM emp e
...
MariaDB [manas_db]> SELECT e.ename, e.job
-> FROM emp e
...
MariaDB [manas_db]> SELECT ename, sal 
-> FROM emp
-> ORDER BY sal DESC
-> LIMIT 5;
+-------+------+
...
MariaDB [manas_db]> SELECT d.dname
-> FROM dept d
-> JOIN emp e
...
MariaDB [manas_db]> SELECT ename, sal, comm, (sal + comm) AS net_pay
-> FROM emp
